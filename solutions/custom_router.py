...
from solutions.pipelines import build_routing_decision_graph, GraphState, MetaRoutingNode, GraphDeps
from solutions.utils import check_available_models, check_deployment_available, DEFAULT_MODEL_ASSIGNMENTS
...
class CustomRouter(BaseRouter):
    def __init__(
        ...
        self.use_meta_routing = use_meta_routing
        self.prompt_version = prompt_version
        self._name = name
        self.graph = build_routing_decision_graph()

        self.model_assignments = DEFAULT_MODEL_ASSIGNMENTS
        if model_assignments is not None:
            self.model_assignments.update(model_assignments)

        self.available_model = check_available_models()
        self.routing_latency: List[float] = []
        self.step_latency: List[Dict[str, float]] = []
        self.cost: List[float] = []
        self.step_cost: List[Dict[str, float]] = []
        self.graph_states: List[GraphState] = []
        self.routing_history: List[Tuple[str, str, str]] = []
        logger.info(f"Available model: {self.available_model}")
        self._executor = ThreadPoolExecutor(max_workers=max_worker)

    @property
    def name(self) -> str:
        if self._name is None:
            return "FullGraphLLMRouter"
        return self._name
    ...
    def route(
        ...
        if available_models is None:
            available_models = self.available_model
        else:
            available_models = list(set(available_models).union(self.available_model))
        try:
            result = self._executor.submit(
                asyncio.run, self._run_route(query, available_models),
            )
            decision, state = result.result()
            decision = check_deployment_available(decision)
        except Exception as e:
            decision = RoutingDecision(
                model_key="gemma-3-12b",
                deployment="cloud",
                confidence=0.0,
                reasoning="Fallback: LLM Graph routing failure",
            )
            state = GraphState(query=query)
            logger.error("Graph routing failed:\n%s", traceback.format_exc())

        self.routing_latency.append(state.routing_latency_ms)
        self.step_latency.append(state.step_latency_ms)
        self.cost.append(state.cost)
        self.step_cost.append(state.step_cost)
    ...
    async def _run_route(self, query: str, available_models: List[str]) -> Tuple[RoutingDecision, GraphState]:
        state = GraphState(
            query=query,
        ...
        deps = GraphDeps(
            ...
            meta_routing=self.use_meta_routing,
            model_assignments=self.model_assignments
        )
        result = await self.graph.run(MetaRoutingNode(), state=state, deps=deps)
        return result.output, result.state

    def save(self, file_path):
        save_dict = {
            "latency": self.routing_latency,
