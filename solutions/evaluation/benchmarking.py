"""
Patched Benchmarking Module for Router Evaluation.

Copied from src/benchmarking.py with one change:
- Imports quality evaluation from solutions/evaluation/quality.py (no tool use required)
  instead of src/quality.py (requires tool use).
- Default evaluator model changed from trinity-mini to gemma-3-27b.
"""
...
from src.latency import get_latency_multiplier
from src.router import BaseRouter, NaiveRouter, StaticRouter

# PATCHED: use our quality module that doesn't require tool use
from solutions.evaluation.quality import (
    QualityEvaluation,
    evaluate_quality,
...
# ── Print utilities (unchanged from src/benchmarking.py) ────────────────────
...
async def benchmark_all_routers(
    ...
    routers: List[BaseRouter] = [
        ...
        StaticRouter("gemma-3n-e4b"),   # Edge static (SMALL model)
        StaticRouter("gemma-3-12b"),  # Cloud static (LARGE model),
        StaticRouter("gemma-3-27b"),
        StaticRouter("nemotron-nano")
    ]
